...
function BetaAgent(numMachines) {
	...
	this.resetWith = function(aInterval, bInterval) {
	...
	this.reset = function() {
		for (var i=0; i<this.alphas.length; i++) {
			this.alphas[i] = 4*Math.random();
			this.betas[i] = 4*Math.random();
		}
	}

	this.update = function(machineIndex, payout) {
		this.alphas[machineIndex] += payout;
		this.betas[machineIndex] += 1-payout;
	}

	this.getMachineToPlay = function() {
